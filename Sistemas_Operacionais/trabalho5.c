...
void* process_1(void* v){
    ...
    sem_post(&procE);//Libera processo 1 para executar E
    sem_post(&procG);//Libera processo 3 para executar G

    sem_wait(&procE);//Liberado pelo processo 1 para executar E
    y = y+2;
    sem_post(&procD);//Libera processo 2 para executar D
    sem_post(&procH);//Libera processo 3 para executar H

    sem_wait(&procF);//Liberado pelo processo 2 para executar F
    sem_wait(&procF1);//Liberado pelo processo 3 para executar F
    u = h+j/3;
...
void* process_2(void* v){
    ...
    x = x*2;
    sem_post(&procC);//Libera processo 2 para executar C

    sem_wait(&procC);//Liberado pelo processo 2 para executar C
    x = x+1;
    sem_post(&procD1);//Libera processo 2 para executar D

    sem_wait(&procD);//Liberado pelo processo 1 para executar D
    sem_wait(&procD1);//Liberado pelo processo 2 para executar D
    h = y+x;
    sem_post(&procF);//Libera processo 1 para executar F
}

void* process_3(void* v){
    sem_wait(&procG);//Liberado pelo processo 1 para executar G
    z=z/2;
    sem_post(&procH1);//Libera processo 3 para executar H

    sem_wait(&procH);//Liberado pelo processo 1 para executar H
    sem_wait(&procH1);//Liberado pelo processo 3 para executar H
    j = z+y-4;
    sem_post(&procF1);
}
